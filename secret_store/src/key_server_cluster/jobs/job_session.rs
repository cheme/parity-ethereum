...
pub mod tests {
	...
	fn job_initialize_fails_if_not_enough_nodes_for_threshold_total() {
		...
		assert_eq!(job.initialize(vec![NodeId::from(1)].into_iter().collect(), None, false).unwrap_err(), Error::ConsensusUnreachable);
	}
	...
	fn job_initialize_fails_if_not_enough_nodes_for_threshold_connected() {
		...
		assert_eq!(job.initialize(vec![NodeId::from(1)].into_iter().collect(), None, false).unwrap_err(), Error::ConsensusTemporaryUnreachable);
	}
	...
	fn job_initialize_fails_if_not_inactive() {
		...
		job.initialize(vec![NodeId::from(1)].into_iter().collect(), None, false).unwrap();
		assert_eq!(job.initialize(vec![NodeId::from(1)].into_iter().collect(), None, false).unwrap_err(), Error::InvalidStateForRequest);
	}
	...
	fn job_initialization_does_not_leads_to_finish_if_single_other_node_is_required() {
		let mut job = JobSession::new(make_master_session_meta(0), SquaredSumJobExecutor, DummyJobTransport::default());
		job.initialize(vec![NodeId::from(2)].into_iter().collect(), None, false).unwrap();
	...
	fn job_response_leads_to_failure_if_too_few_nodes_left() {
		let mut job = JobSession::new(make_master_session_meta(1), SquaredSumJobExecutor, DummyJobTransport::default());
		job.initialize(vec![NodeId::from(1), NodeId::from(2)].into_iter().collect(), None, false).unwrap();
		assert_eq!(job.state(), JobSessionState::Active);
	...
	fn job_response_succeeds() {
		...
		job.initialize(vec![NodeId::from(1), NodeId::from(2), NodeId::from(3)].into_iter().collect(), None, false).unwrap();
		assert_eq!(job.state(), JobSessionState::Active);
		assert!(!job.is_result_ready());
	...
	fn job_broadcasts_self_response() {
		let mut job = JobSession::new(make_master_session_meta(1), SquaredSumJobExecutor, DummyJobTransport::default());
		job.initialize(vec![NodeId::from(1), NodeId::from(2)].into_iter().collect(), None, true).unwrap();
		assert_eq!(job.state(), JobSessionState::Active);
	...
	fn job_fails_with_temp_error_if_more_than_half_nodes_respond_with_temp_error() {
		let mut job = JobSession::new(make_master_session_meta(2), SquaredSumJobExecutor, DummyJobTransport::default());
		job.initialize(vec![NodeId::from(1), NodeId::from(2), NodeId::from(3), NodeId::from(4)].into_iter().collect(), None, false).unwrap();
