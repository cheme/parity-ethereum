...
pub mod tests {
	...
	fn node_added_using_servers_set_change() {
		...
		let nodes_to_add: BTreeSet<_> = (0..1).map(|_| Random.generate().unwrap().public().as_ref().into()).collect();
	...
	fn node_added_using_server_set_change_from_this_node() {
		...
		let nodes_to_add: BTreeSet<NodeId> = (0..1).map(|_| Random.generate().unwrap().public().as_ref().into()).collect();
